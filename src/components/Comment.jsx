...
export default function Comment({
  ...
  const commentDetail = useQuery({
    ...
    queryFn: () => getDetailComment(commentId),
    retry: false,
  })

  const markupDetail = useQuery({
    queryKey: ["markupDetail", commentId],
    queryFn: () => getMarkUp(commentId),
  })

  const voteDetail = useQuery({
    queryKey: ["voteDetail", commentId],
    queryFn: () => getVote(commentId),
  })

  const shareDetail = useQuery({
    queryKey: ["shareDetail", commentId],
    queryFn: () => getShare(commentId),
  })
  ...
  return (
    <>
      ...
      <div className="w-full h-fit flex flex-col gap-2 pb-4 bg-gray-50 border-b-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700">
        ...
        {commentDetail.isSuccess && (
          <>
            {commentDetail.data.isSoftDeleted ? (
              ...
              <>
                <NavLink to={`/home/comment/${commentId}`}>
                  <div
                    className="flex flex-col gap-2 pl-4 pt-4 pr-4 hover:bg-gray-100 cursor-pointer dark:hover:bg-gray-600"
                    // onClick={() => {
                    //   navigation(`/home/comment/${commentId}`)
                    // }}
                  >
                                  ...
                                  {profile.isSuccess &&
                                    profile.data._id ===
                                      ...
                                      <div className="">
                                        ...
                                        >
                                          <p className="block w-28 text-gray-700 dark:text-gray-200 dark:hover:bg-blue-600">
                                            Delete Comment
                                          </p>
                                        </div>
                                      </div>
                  ...
                  <div className="flex flex-row gap-2 items-center">
                    <div
                      ...
                      onClick={() => {
                        ...
                        setCommentId({ commentId: commentId })
                      }}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
