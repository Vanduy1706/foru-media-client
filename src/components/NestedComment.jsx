...
export default function NestedComment({
  ...
  const markupDetail = useQuery({
    queryKey: ["markupDetail", reply._id],
    queryFn: () => getMarkUp(reply._id),
    enabled: !!reply._id,
  })

  const voteDetail = useQuery({
    queryKey: ["voteDetail", reply._id],
    queryFn: () => getVote(reply._id),
  })

  const shareDetail = useQuery({
    queryKey: ["shareDetail", reply._id],
    queryFn: () => getShare(reply._id),
  })
  ...
  const commentDetail = useQuery({
    queryKey: ["commentDetail", reply._id],
    queryFn: () => getDetailComment(reply._id),
    retry: false,
  })
  ...
  const voteNestedReply = useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.setQueryData(["nestedReply", commentDetailId], (old) => {
        if (!old) return
        return old.map((reply) => {
          if (reply._id === data.vote._id) {
            return data.vote
          } else {
            return reply
          }
  ...
  const shareNestedReply = useMutation({
    mutationFn: shareSpeechOrComment,
    onSuccess: (data, variables) => {
      queryClient.setQueryData(["nestedReply", commentDetailId], (old) => {
        if (!old) return
        return old.map((reply) => {
          if (reply._id === data.share._id) {
            return data.share
          } else {
            return reply
          }
  ...
  return (
    <div
      className="w-full h-fit flex flex-col gap-2 py-4 border-b-2 border-gray-300 relative dark:border-gray-600"
      key={reply._id}
    >
      {commentDetail.isPending && (
        <div className="skeleton-paragraph dark:skeleton-paragraph-dark"></div>
      )}
      {commentDetail.isError && <p>Something is wrong</p>}
        ...
        <>
          {commentDetail.data.isSoftDeleted ? (
            ...
            <>
              ...
              <NavLink to={`/home/comment/${commentDetail.data._id}`}>
                <div
                  className="flex flex-col gap-2 hover:bg-gray-100 cursor-pointer dark:hover:bg-gray-600"
                  // onClick={() => {
                  //   navigation(`/home/comment/${commentDetail.data._id}`)
                  // }}
                >
                    ...
                    <div className="flex flex-col">
                      <p className="w-52 font-inter font-semibold text-gray-700 break-words dark:text-gray-200">
                        {commentDetail.data.author.username.length >= 30
                          ? `${commentDetail.data.author.username.slice(
                              0,
                              30
                            )}...`
                      ...
                      <p className="w-52 font-inter font-medium text-gray-500 break-words dark:text-gray-400">
                        ...
                        {commentDetail.data.author.nickname.length >= 30
                          ? `${commentDetail.data.author.nickname.slice(
                              0,
                              30
                            )}...`
                          ...
                          {profile.isSuccess &&
                            profile.data._id ===
                              ...
                              <div className="">
                                ...
                                >
                                  <p className="block w-28 dark:text-gray-200">
                                    Update comment
                                  </p>
                                </div>
                                <div
                ...
                <div className="flex flex-row gap-2 items-center">
                  <div
                    ...
                    onClick={() => {
                      ...
                      setCommentId({ commentId: reply._id })
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="px-4 pt-2">
                <p className="font-inter font-medium text-gray-600 dark:text-gray-300">
                  &bull; {formatter(reply.createdAt)}
                </p>
              </div>
            </>
          )}
