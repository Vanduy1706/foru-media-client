...
import { getReplyNested } from "../../apis/comment/getReplyNested"
...
import { getReplyOnComment } from "../../apis/comment/getReplyOnComment"
...
export default function CommentDetail() {
  ...
  const setCommentId = useComment()
  const queryClient = useQueryClient()
  // const [toggle, setToggle] = useState(false)
  // const [toggleCommentDetail, setToggleCommentDetail] = useState(false)
  ...
  const commentDetail = useQuery({
    ...
    queryFn: () => getDetailComment(id),
    retry: false,
  })

  const speechDetail = useQuery({
    queryKey: ["speechDetail", commentDetail.data?.parentSpeech],
    queryFn: () => getDetailSpeech(commentDetail.data.parentSpeech),
    enabled: !!commentDetail.data?.parentSpeech,
    staleTime: 1000 * 60 * 1,
    retry: false,
  })

  const nestedReply = useQuery({
    queryKey: ["nestedReply", id],
    queryFn: () => getReplyNested(id),
    enabled: !!id,
    retry: false,
    staleTime: 1000 * 60 * 1,
  })
  ...
  const replies = useQuery({
    queryKey: [
      "replies",
      {
        commentId: commentDetail.data?._id,
    ...
    enabled: !!commentDetail.data?._id && !!commentDetail.data?.parentSpeech,
    staleTime: 1000 * 60 * 1,
    retry: false,
  })
  ...
  const results = useQueries({
    queries: [
      ...
      {
        queryKey: ["markupDetail", speechDetail.data?._id],
        queryFn: () => getMarkUp(speechDetail.data?._id),
        enabled: !!speechDetail.data?._id,
      },
    ],
  })

  const [markupCommentDetail, markupSpeechDetail] = results
  ...
  const voteQueries = useQueries({
    queries: [
      ...
      {
        queryKey: ["voteDetail", speechDetail.data?._id],
        queryFn: () => getVote(speechDetail.data._id),
        enabled: !!speechDetail.data?._id,
        retry: false,
      },
    ],
  ...
  const [voteCommentDetail, voteSpeechDetail] = voteQueries
  ...
  const shareQueries = useQueries({
    queries: [
      ...
      {
        queryKey: ["shareDetail", speechDetail.data?._id],
        queryFn: () => getShare(speechDetail.data._id),
        enabled: !!speechDetail.data?._id,
        retry: false,
      },
    ],
  ...
  const [shareCommentDetail, shareSpeechDetail] = shareQueries
  // const voteNestedReply = useMutation({
  //   mutationFn: voteSpeechOrComment,
  //   onSuccess: (data, variables) => {
  //     queryClient.setQueryData(
  //       ["nestedReply", commentDetail.data?._id],
  //       (old) => {
  //         if (!old) return
  ...
  return (
    <div className="w-full flex flex-col overflow-y-scroll">
      ...
      {speechDetail.isError && (
        <div>
          <p>{speechDetail.error.message}</p>
        </div>
      )}
      {speechDetail.isSuccess && (
        <div className="w-full h-fit flex flex-col gap-2 py-4 border-b-2 border-gray-300 relative dark:border-gray-600">
          {speechDetail.data.isSoftDeleted ? (
            ...
            <>
              <div className="w-1 h-full absolute left-2 -bottom-4 bg-gray-400 rounded dark:bg-gray-500"></div>
              <div className="w-4 h-4 absolute left-0.5 top-1 bg-gray-400 rounded-full dark:bg-gray-500"></div>
              <div className="w-2 h-2 absolute left-1.5 top-2 bg-gray-50 rounded-full dark:bg-gray-200"></div>
              <div className="w-4 h-4 absolute left-0.5 -bottom-5 bg-gray-400 rounded-full dark:bg-gray-500"></div>
              <div className="w-2 h-2 absolute left-1.5 -bottom-4 bg-gray-50 rounded-full dark:bg-gray-200"></div>
              <NavLink to={`/home/speech/${speechDetail.data._id}`}>
                <div
                  className="flex flex-col gap-2 pt-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600"
                  // onClick={() => {
                  //   navigation(`/home/speech/${speechDetail.data._id}`)
                  // }}
                >
                  <div className="w-full h-fit px-4 flex flex-row items-center justify-between">
                    ...
                    <div className="relative">
                      <div
                        ...
                        onClick={(e) => {
                          e.preventDefault()
                          e.stopPropagation()
                          // toggleHandler(speechDetail.data._id)
                          setToggle((prev) =>
                  ...
                  <div className="font-inter font-normal text-gray-700 px-4 text-justify dark:text-gray-200">
                    <p>{speechDetail.data.content}</p>
                  </div>
                </div>
              </NavLink>
              <div className="flex flex-row justify-between pt-2 px-4">
                <div className="flex flex-rows gap-2 items-center">
                  ...
                  >
                    {voteSpeechDetail.isSuccess && voteSpeechDetail.data ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                ...
                <div className="flex flex-row gap-2 items-center">
                  ...
                  >
                    {shareSpeechDetail.isSuccess && shareSpeechDetail.data ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="15"
                ...
                <div className="flex flex-row gap-2 items-center cursor-pointer">
                  {markupSpeechDetail.isPending && <></>}
                  {markupSpeechDetail.isSuccess && markupSpeechDetail.data ? (
                    <>
                      <div
                        onClick={() => {
                          markup.mutate({
      ...
      {/* {nestedReply.isError && (
        <div>
          <p>{nestedReply.error.message}</p>
        </div>
      )} */}
      {nestedReply.isSuccess &&
        nestedReply.data.map((reply, index) => (
          <Fragment key={reply._id}>
            <NestedComment
              reply={reply}
              // toggle={toggle}
              // toggleHandler={toggleHandler}
              commentDetailId={commentDetail.data?._id}
            />
          </Fragment>
        ))}
      {commentDetail.isPending && (
        <div className="skeleton-paragraph dark:skeleton-paragraph-dark"></div>
      )}
      {commentDetail.isError && (
        <div>
          <p>{commentDetail.error.message}</p>
        </div>
      )}
      {commentDetail.isSuccess && (
        <div className="w-full h-fit flex flex-col gap-2 py-4 border-b-2 border-gray-300 relative dark:border-gray-600">
          {commentDetail.data.isSoftDeleted ? (
            <p className="font-inter font-semibold text-gray-700 text-xl px-4 dark:text-gray-200">
              This speech is deleted
            </p>
          ) : (
            <>
              <div className="w-full h-fit px-4 flex flex-row items-center justify-between">
                <div className="flex flex-col">
                  <p className="w-52 font-inter font-semibold text-gray-700 break-words dark:text-gray-200">
                    {commentDetail.data.author.username.length >= 30
                      ? `${commentDetail.data.author.username.slice(0, 30)}...`
                      : `${commentDetail.data.author.username}`}
                  </p>
                  <p className="w-52 font-inter font-medium text-gray-500 break-words dark:text-gray-400">
                    @
                    {commentDetail.data.author.nickname.length >= 30
                      ? `${commentDetail.data.author.nickname.slice(0, 30)}...`
                      : `${commentDetail.data.author.nickname}`}
                  </p>
                </div>
                <div className="relative">
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      e.stopPropagation()
                      setToggle((prev) =>
                        prev === commentDetail.data._id
                          ? null
                          : commentDetail.data._id
                      )
                    }}
                  >
                    <div className="px-4 py-2 hover:bg-blue-100 rounded-full dark:hover:bg-blue-700">
                  ...
                  {toggle === commentDetail.data._id && (
                    <div className="flex flex-col py-4 bg-blue-100 rounded absolute top-8 left-0 -translate-x-full dark:bg-blue-700">
                      {profile.isSuccess &&
                        profile.data._id === commentDetail.data.author._id && (
                          <div className="">
                            <div
                              className="py-2 px-4 hover:bg-blue-50 cursor-pointer dark:hover:bg-blue-600"
                              onClick={(e) => {
                                setOpenForm("UpdateComment")
                                setCommentId({
                                  commentId: commentDetail.data._id,
              ...
              <div className="font-inter font-normal text-gray-800 px-4 text-justify dark:text-gray-200">
                <p>{commentDetail.data.content}</p>
              </div>
              <div className="flex flex-row justify-between pt-2 px-4">
                <div className="flex flex-rows gap-2 items-center">
                  <div
                    ...
                    onClick={(e) => {
                      vote.mutate({
                        targetType: "comment",
                        targetId: commentDetail.data._id,
                      })
                    }}
                  >
                    {voteCommentDetail.isSuccess && voteCommentDetail.data ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                ...
                <div className="flex flex-row gap-2 items-center">
                  <div
                    className="cursor-pointer"
                    onClick={() => {
                      setOpenForm("CreateReply")
                      setCommentId({ commentId: commentDetail.data._id })
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <p className="font-inter font-normal text-base text-gray-700 dark:text-gray-200">
                    {commentDetail.data.commentCount}
                  </p>
                </div>
                <div className="flex flex-row gap-2 items-center">
                  <div
                  ...
                  >
                    {shareCommentDetail.isSuccess && shareCommentDetail.data ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="15"
                  ...
                  <p className="font-inter font-normal text-base text-gray-700 dark:text-gray-200">
                    {commentDetail.data.shareCount}
                  </p>
                </div>
                <div className="flex flex-row gap-2 items-center cursor-pointer">
                  {markupCommentDetail.isPending && <></>}
                  {markupCommentDetail.isSuccess && markupCommentDetail.data ? (
                    <>
                      <div
                        onClick={() => {
                          markup.mutate({
              ...
              <div className="px-4 pt-2">
                <p className="font-inter font-medium text-gray-600 dark:text-gray-300">
                  &bull; {formatter(commentDetail.data.createdAt)}
                </p>
              </div>
            </>
          )}
